...
window.addEventListener('load', function() {
  ...
  // add buttons
  document.getElementById('btn-add-student').onclick = function() { editType=''; openModal('student'); resetForm('student'); };
  document.getElementById('btn-add-program').onclick  = function() { editType=''; openModal('program'); resetForm('program'); };
  document.getElementById('btn-add-college').onclick  = function() { editType=''; openModal('college'); resetForm('college'); };
  ...
  document.getElementById('student-search').oninput = function() {
    ...
    renderTable('student', students.filter(function(s) { return (s.ID+s.FirstName+s.LastName+s.ProgramCode).toLowerCase().includes(q); }));
  };
  document.getElementById('program-search').oninput = function() {
    var q = this.value.toLowerCase();
    renderTable('program', programs.filter(function(p) { return (p.Code+p.Name+p.CollegeCode).toLowerCase().includes(q); }));
  };
  document.getElementById('college-search').oninput = function() {
    var q = this.value.toLowerCase();
    renderTable('college', colleges.filter(function(c) { return (c.Code+c.Name).toLowerCase().includes(q); }));
  };
});

function loadAll() {
  go.ListColleges().then(function(d) { colleges = d||[]; renderTable('college', colleges); fillDropdown('f-program-college', colleges, 'Code', 'Name'); });
  go.ListPrograms().then(function(d) { programs = d||[]; renderTable('program', programs); fillDropdown('f-student-program', programs, 'Code', 'Name'); });
...
function renderTable(type, data) {
  ...
  if (!data || data.length === 0) { wrap.innerHTML = '<p class="empty">No ' + type + 's found</p>'; return; }
  ...
  var label = type === 'student' ? function(r) { return r.FirstName+' '+r.LastName; } : function(r) { return r.Name; };
  ...
  data.forEach(function(row) {
    html += '<tr>';
    cols.forEach(function(c) { html += '<td>' + (row[c[0]]||'') + '</td>'; });
    html += '<td>'
      + '<button class="btn-edit" onclick="openEdit(\'' + type + '\',\'' + row[key] + '\')">Edit</button>'
      + '<button class="btn-delete" onclick="askDelete(\'' + type + '\',\'' + row[key] + '\',\'' + label(row) + '\')">Delete</button>'
      + '</td></tr>';
  });
...
function fillDropdown(id, data, valKey, labelKey) {
  ...
  data.forEach(function(d) { sel.innerHTML += '<option value="' + d[valKey] + '">' + d[valKey] + ' - ' + d[labelKey] + '</option>'; });
  sel.value = prev;
}
...
function resetForm(type) {
  document.querySelectorAll('#modal-' + type + ' input, #modal-' + type + ' select').forEach(function(el) { el.value = ''; });
...
function submitStudent() {
  ...
  if (!id||!fn||!ln||!pr||!yr||!gn) { showErr('student-id','All fields required'); return; }
  ...
  p.then(function() { toast('Student saved'); closeModal('student'); go.ListStudents().then(function(d) { students=d||[]; renderTable('student',students); }); })
...
function submitProgram() {
  ...
  if (!code||!name||!col) { showErr('program-code','All fields required'); return; }
  ...
  p.then(function() { toast('Program saved'); closeModal('program'); go.ListPrograms().then(function(d) { programs=d||[]; renderTable('program',programs); fillDropdown('f-student-program',programs,'Code','Name'); }); })
...
function submitCollege() {
  ...
  if (!code||!name) { showErr('college-code','All fields required'); return; }
  ...
  p.then(function() { toast('College saved'); closeModal('college'); go.ListColleges().then(function(d) { colleges=d||[]; renderTable('college',colleges); fillDropdown('f-program-college',colleges,'Code','Name'); }); })
   .catch(function(e) { showErr('college-code', e); });
}

function askDelete(type, key, label) {
  document.getElementById('confirm-text').innerHTML = 'Delete <strong>' + label + '</strong>?';
  deleteFn = function() {
    var p = type==='student' ? go.DeleteStudent(key) : type==='program' ? go.DeleteProgram(key) : go.DeleteCollege(key);
    p.then(function() { toast(type+' deleted'); closeModal('confirm'); loadAll(); })
